...
static async hoDashboard(req) {
  ...
  const ops = await DailyOperations.find(query)
    .populate(['branch', 'cashbook1', 'cashbook2'])
    .lean();
  ...
  ops.forEach(op => {
    ...
    if (op.branch?._id) {
      summaryAccumulator.activeBranches.add(op.branch._id.toString());
    }
    ...
    if (!branchStats.lastOperation || op.date > branchStats.lastOperation) {
      branchStats.lastOperation = op.date;
    }
  });
  ...
  // Recalculate using real Mongoose documents
  for (const rollData of disbursementRolls) {
    const roll = await DisbursementRoll.findById(rollData._id);
    if (roll) {
      await roll.calculateCumulativeDisbursement();
  ...
  // HO-wide cumulative disbursement roll number should be the MAX, not SUM
  summaryAccumulator.totalDisbursementRollNo = disbursementRolls.reduce(
    (max, roll) => {
      const value = roll.disNo || 0;
      return value > max ? value : max;
  ...
  const consolidatedSummary = summaryAccumulator.totalOperations
    ? {
        ...
        totalCollections:
          ...
          summaryAccumulator.totalSavings +
          summaryAccumulator.totalCharges,
        activeBranches: Array.from(
          summaryAccumulator.activeBranches
        ),
        totalOperations: summaryAccumulator.totalOperations
      }
    : {
